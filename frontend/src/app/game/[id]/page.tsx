...
export default function GameDetail({ params }: { params: { id: string } }) {
  ...
  const handleTransaction = async () => {
    if (!gameUserId || !selectedProduct || !paymentMethod) {
      alert("Harap lengkapi User ID, Nominal, dan Metode Pembayaran!");
      return;
    }
        ...
        <div className="absolute inset-0 z-20 flex items-center px-6">
           <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row items-center md:items-end gap-6 pb-8 md:pb-12 text-center md:text-left pt-20 md:pt-0">
              <motion.div 
                ...
                className="w-32 h-32 md:w-40 md:h-40 relative rounded-2xl overflow-hidden shadow-[0_0_30px_rgba(20,184,166,0.3)] border-2 border-brand-500/50"
              >
                <Image src={game.imageUrl} alt={game.name} fill className="object-cover" />
              </motion.div>
              <div className="mb-2">
                <h1 className="text-3xl md:text-5xl font-extrabold text-theme-text mb-2">{game.name}</h1>
                <p className="text-brand-300 font-medium tracking-wide uppercase">{game.publisher}</p>
              </div>
           </div>
        </div>
      </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-10 relative z-30">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <div className="absolute -left-4 -top-4 w-10 h-10 rounded-full bg-gradient-to-br from-brand-400 to-brand-600 border-4 border-dark-bg flex items-center justify-center font-bold text-white shadow-lg">
                <UserSquare2 size={18} />
              </div>
              <h2 className="text-xl items-center font-bold mb-6 text-theme-text border-b border-dark-border pb-4">Masukkan ID Game</h2>
              <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1 group">
                  <label className="text-sm text-theme-muted mb-1 block group-focus-within:text-brand-400 transition-colors">User ID</label>
                  <input 
                    type="text" 
                    value={gameUserId}
                    onChange={(e) => setGameUserId(e.target.value)}
                    placeholder="Contoh: 12345678"
                    className="w-full bg-dark-bg border border-dark-border rounded-xl px-4 py-3 text-theme-text focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all font-mono"
                  />
                </div>
                <div className="flex-1 group">
                  <label className="text-sm text-theme-muted mb-1 block group-focus-within:text-brand-400 transition-colors">Zone ID (Opsional)</label>
                  <input 
                    type="text" 
                    value={gameZoneId}
            ...
            >
              ...
              <h2 className="text-xl font-bold mb-6 text-theme-text border-b border-dark-border pb-4">Pilih Nominal</h2>
